...
resource "google_bigquery_table" "data_table" {
  ...
  external_data_configuration {
    ...
    source_uris   = ["gs://${google_storage_bucket.data_bucket.name}/fatos/ena/ano=*"]
    hive_partitioning_options {
      mode              = "AUTO"
      source_uri_prefix = "gs://${google_storage_bucket.data_bucket.name}/fatos/ena"
    }
  }
}
# --- NOVA TABELA: Tabela de Dimensão de Reservatórios ---

resource "google_bigquery_table" "data_table_dimensao" {
  project    = var.project_id
  dataset_id = google_bigquery_dataset.bq_projeto.dataset_id
  table_id   = "reservatorios_dimensao" # Nome da nova tabela no BigQuery

  # Configuração para ler dados externos do Google Storage
  external_data_configuration {
    # Habilitamos a autodetecção de esquema para esta tabela
    autodetect    = true
    source_format = "PARQUET" # Assumindo que os arquivos também são Parquet
    
    # Aponta para o novo caminho no bucket, pegando todos os arquivos da pasta
    source_uris   = ["gs://${google_storage_bucket.data_bucket.name}/dimensao/reservatorios/*"]
  }
}
